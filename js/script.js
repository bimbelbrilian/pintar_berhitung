...
function resetSelections() {
    // Reset UI selections
    document.querySelectorAll('.operation-card, .level-card').forEach(card => {
...
function displayQuestion(questionHTML) {
    ...
    // HAPUS ini: flipQuestion(questionHTML);
    // HAPUS ini: questionDisplay.innerHTML = questionHTML;
...
function createLevelCards() {
    ...
    // Create level cards - TANPA bagian level-difficulty
    LEVELS.forEach(level => {
        const card = document.createElement('div');
        card.className = 'level-card';
        card.dataset.levelId = level.id;
        card.tabIndex = 0; // Untuk accessibility
        card.setAttribute('role', 'button');
        card.setAttribute('aria-label', `Level ${level.id}: ${level.range}`);
        
        // HTML TANPA level-difficulty
        card.innerHTML = `
    ...
    console.log(`✅ Created ${LEVELS.length} level cards (without stars)`);
    
    // Setup event delegation untuk performance
    levelClickHandler = (e) => {
...
function initAudio() {
    ...
    try {
        ...
        // Pre-create oscillator types untuk performance
        // (Tidak perlu buffer, kita gunakan oscillator langsung)
    } catch (error) {
...
function setupBackButtons() {
    ...
    if (backLevelBtn) {
        const handler = () => {
            ...
            if (gameState.isPlaying) {
                console.log("   Game is playing, showing modal");
                showCustomModal();
            } else {
                console.log("   Game not playing, going to menu");
                // INI YANG DIPERBAIKI: panggil goBackToMenu() bukan goBackToLevelSelection()
                goBackToMenu();
            }
        };
        
        backLevelBtn.addEventListener('click', handler);
        eventListeners.push({ element: backLevelBtn, type: 'click', handler });
    }
}
...
window.debugScreens = function() {
    ...
    const screenIds = ['menuScreen', 'levelScreen', 'gameScreen', 'loadingScreen'];
...
window.testNavigation = function() {
    ...
    if (firstCard) {
        ...
        console.error("No operation cards found!");
    }
};
...
window.debugGameState = function() {
    ...
    console.log("isPlaying:", gameState.isPlaying);
    console.log("Selected Answers:", gameState.selectedAnswers.length);
    console.log("Score:", gameState.score);
    console.log("Correct Answers:", gameState.correctAnswers);
    console.log("Time Left:", gameState.timeLeft);
    console.log("Timer Interval:", gameState.timerInterval ? "Running" : "Stopped");
    console.log("Selected Operation:", gameState.selectedOperation);
    console.log("Selected Level:", gameState.selectedLevel);
    console.log("Sound Enabled:", gameState.soundEnabled);
    console.log("========================================");
};

// Debug operation modules
window.debugOperations = function() {
    console.log("🔧 ========== OPERATIONS DEBUG ==========");
    const ops = Object.keys(window.gameOperations);
    ...
    ops.forEach(opId => {
        ...
        console.log(`  Icon: ${op.icon || 'N/A'}`);
        console.log(`  Description: ${op.description || 'N/A'}`);
        console.log(`  generateQuestion: ${typeof op.generateQuestion}`);
        console.log(`  generateWrongAnswers: ${typeof op.generateWrongAnswers}`);
    });
    console.log("========================================");
};

// Test question generation
window.testQuestion = function(operationId = null, levelId = 1) {
    console.log("🧪 ========== TEST QUESTION ==========");
    
    const opId = operationId || gameState.selectedOperation || 'penjumlahan';
    const level = LEVELS.find(l => l.id === levelId) || LEVELS[0];
    
    console.log(`Testing: ${opId}, Level: ${level.range}`);
    
    const operation = window.gameOperations[opId];
    if (!operation) {
        console.error(`❌ Operation ${opId} not found`);
        return;
    }
    
    if (typeof operation.generateQuestion !== 'function') {
        console.error(`❌ generateQuestion not a function for ${opId}`);
        return;
    }
    
    try {
        const question = operation.generateQuestion(level);
        console.log("Generated question:", question);
        
        // Test wrong answers
        if (operation.generateWrongAnswers) {
            const wrongAnswers = operation.generateWrongAnswers(question.correctAnswer, level);
...
// Auto debug setiap state change (optional, hanya untuk development)
if (window.location.href.includes('debug=true')) {
    const originalIsPlaying = Object.getOwnPropertyDescriptor(gameState, 'isPlaying');
    Object.defineProperty(gameState, 'isPlaying', {
        get() {
            return originalIsPlaying ? originalIsPlaying.get.call(this) : this._isPlaying;
        },
        set(value) {
            console.log(`🔄 isPlaying changed: ${this.isPlaying} -> ${value}`, new Error().stack);
            if (originalIsPlaying && originalIsPlaying.set) {
                originalIsPlaying.set.call(this, value);
            } else {
...
window.addEventListener('error', function(e) {
    ...
    // Don't show alert for minor errors during gameplay
    if (!gameState.isPlaying || e.message.includes('ResizeObserver')) {
        return;
    }
    
    // Show user-friendly error
    showErrorToast(`Terjadi kesalahan: ${e.message}`);
});
...
window.addEventListener('unhandledrejection', function(e) {
    ...
    // Log ke analytics
    if (window.trackError) {
        window.trackError(e.reason);
    }
});

// Function untuk menampilkan error toast
...
window.addEventListener('online', function() {
    ...
    showErrorToast("Koneksi internet pulih", 3000);
});

window.addEventListener('offline', function() {
    console.log("🌐 Network: Offline");
    showErrorToast("Koneksi internet terputus", 0); // 0 = tidak auto-hide
});
...
document.addEventListener('visibilitychange', function() {
    ...
    if (document.visibilityState === 'hidden' && gameState.isPlaying) {
        // Auto-pause game jika user switch tab
        if (gameState.timerInterval) {
            clearInterval(gameState.timerInterval);
            gameState.timerInterval = null;
        }
...
// Juga listen untuk window load
window.addEventListener('load', function() {
    console.log("🖼️ Window Loaded");
    
    // Jika DOM belum ready, tunggu DOMContentLoaded
    if (domReady) {
        checkModulesAndInit();
    }
});
