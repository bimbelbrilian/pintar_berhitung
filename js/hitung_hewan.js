// File: hitung_hewan.js - OPERASI MENGHITUNG HEWAN
(function() {
    const operation = {
        id: 'hitung_hewan',
        ...
        getLevelConfig: function(levelNumber) {
            const configs = [
                // Level 1: 1-5 hewan, jawaban 1-5
                { min: 1, max: 5, answerMin: 1, answerMax: 5 },
                // Level 2: 5-10 hewan, jawaban 5-10
                { min: 5, max: 10, answerMin: 5, answerMax: 10 },
                // Level 3: 10-15 hewan, jawaban 10-15
                { min: 10, max: 15, answerMin: 10, answerMax: 15 },
                // Level 4: 11-20 hewan, jawaban 15-20
                { min: 11, max: 20, answerMin: 15, answerMax: 20 },
                // Level 5: 20-30 hewan, jawaban 20-30
        ...
        generateQuestion: function(level) {
            ...
            // Generate jumlah hewan sesuai level
            const count = Math.floor(Math.random() * (config.max - config.min + 1)) + config.min;
            
            // Pilih hewan secara acak untuk ditampilkan
            ...
            console.log(`🎯 ATURAN BARIS HEWAN: ${count} icon = ${rows} baris, ${iconsPerRow} icon/baris`);
            
            // Generate tampilan hewan dengan baris
            let animalDisplay = '';
            let remainingAnimals = count;
            
            for (let rowIndex = 0; rowIndex < rows; rowIndex++) {
                // Jumlah hewan di baris ini
                const animalsInThisRow = Math.min(iconsPerRow, remainingAnimals);
                
                // Buat string hewan untuk baris ini
                let rowContent = '';
                for (let iconIndex = 0; iconIndex < animalsInThisRow; iconIndex++) {
                    rowContent += `<span class="counting-icon">${animal}</span>`;
                }
                
                animalDisplay += `<div class="counting-row row-${rowIndex + 1}">${rowContent}</div>`;
